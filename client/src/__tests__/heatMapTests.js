...
describe("Heatmap Test", () => {
    ...
    it("create heatmap1", async () => {
        const mapName = "heatmap1";
        const mapResponse = await apis.createMap(mapData, username, mapName, mapType, token);
        heatmap1id = mapResponse.data.mapData._id;
    ...
    it("Fail to retrieve heatmaps with invalid token", async () => {
    ...
    it("Fail to retrieve heatmaps for an unregistered user", async () => {
    ...
    it("Fail to retrieve heatmaps with token for another user", async () => {
    ...
    it("Fail to retrieve heatmaps with expired token", async () => {
    ...
    it("Fail retrieval of non-existent heatmap", async () => {
    ...
    //         title: "Updated Forked Heatmap1",
    //         mapData: Buffer.from(Object.values("updated test data")),
    //         author: username,
    //         mapType: mapType
    //     };

    //     const mapResponse = await apis.updateMap(forkedheatmap1id, updatedMapData, token);
    //     forkedheatmap1id = mapResponse.data.mapData.id;
    ...
    it("create heatmap2", async () => {
        const mapName = "heatmap2";
        const mapResponse = await apis.createMap(mapData, username, mapName, mapType, token);
        heatmap2id = mapResponse.data.mapData._id;
        expect(mapResponse.status).toEqual(201); // Status 201 : Map Created
    });

    it("Fail to update a heatmap2 with invalid data", async () => {
        const invalidUpdateData = null; // Example of invalid data
        await expect(apis.updateMap(heatmap2id, invalidUpdateData, token))
            .rejects
            .toEqual(expect.objectContaining({
                response: expect.objectContaining({
                    status: 400
    ...
    it("delete heatmap1 id", async () => {
        const mapResponse = await apis.deleteMap(heatmap1id, token);
        expect(mapResponse.status).toEqual(200); // Status 200 : Map Deleted
    });
