...
const TravelMap = (props) => {
  ...
  const startHere = (e) => {
    if (routeControlRef.current) {
      ...
      console.log(
        "routeControlRef.current.getWaypoints" +
          routeControlRef.current.getWaypoints().map((wp) => wp.latLng)
      );
      console.log(
        "routeControlRef.current.Waypoints" + routeControlRef.current.Waypoints
      );
    }
  };
  ...
  const goHere = (e) => {
    if (routeControlRef.current) {
      routeControlRef.current.spliceWaypoints(
        routeControlRef.current.getWaypoints().length - 1,
        1,
        e.latlng
      );
    ...
    if (store.currentMapObject.mapProps?.waypoints) {
      console.log(
        "store.currentMapObject.mapProps" +
          store.currentMapObject.mapProps.waypoints
      );
  ...
  const runDirection = async () => {
    ...
    const geocode = async (address) => {
      const url = `https://www.mapquestapi.com/geocoding/v1/address?key=S8d7L47mdyAG5nHG09dUnSPJjreUVPeC&location=${encodeURIComponent(
        address
      )}`;
    ...
    try {
      ...
      const routingControl = L.Routing.control({
        ...
        createMarker: function (i, waypoint, n) {
          const markerIcon =
            i === 0 ? startIcon : i > 0 && i < n - 1 ? inBetweenIcon : endIcon;
